...
public final class AssetTransfer implements ContractInterface {
     ...
     */
    @Transaction(intent = Transaction.TYPE.EVALUATE)
    public String GetAllAssets(final Context ctx, final String owner) {
        ChaincodeStub stub = ctx.getStub();
        List<Asset> queryResults = new ArrayList<Asset>();
        /**couchDb*/
        String query = "{\"selector\":{\"docType\":\"asset\",\"owner\":" + owner + "}";
    ...
    @Transaction(intent = Transaction.TYPE.EVALUATE)
    public String GetAssetsByCouchDb(final Context ctx) {
        ChaincodeStub stub = ctx.getStub();
    ...
    @Transaction(intent = Transaction.TYPE.EVALUATE)
    public String GetAssetHistoryByKey(final Context ctx,final String assetId) {
        ChaincodeStub stub = ctx.getStub();

        List<Asset> queryResults = new ArrayList<Asset>();
        QueryResultsIterator<KeyModification> historys = stub.getHistoryForKey(assetId);

        for (KeyModification history: historys) {
            Asset asset = genson.deserialize(history.getStringValue(),Asset.class);
            queryResults.add(asset);
            System.out.println(asset.toString());
        }
        final String response = genson.serialize(queryResults);
